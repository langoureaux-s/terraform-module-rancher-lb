...
locals {
  ...
  section_hostname  = "${join("\n", formatlist("- protocol: %s\n  service: %s\n  source_port: %s\n  target_port: %s", var.test, var.services, var.sources_port, var.targets_port))}"
  section_without_hostname = "${join("\n", formatlist("- protocol: %s\n  service: %s\n  source_port: %s\n  target_port: %s", var.protocols, var.services, var.sources_port, var.targets_port))}"
  port_rules        = "${length(var.test) > 0 ? "${local.section_hostname}"  : "${local.section_without_hostname}"}"
